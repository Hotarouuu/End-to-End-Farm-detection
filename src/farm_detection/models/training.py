...
def train():
    ...
    with mlflow.start_run():
        ...
        logging.info("Data split completed. Training set shape: {}, Testing set shape: {}".format(train_X.shape, test_X.shape))
        logging.info("Initializing the Naive Bayes model with priors: {} and var_smoothing: {}".format(config["model"]["variables"]["priors"], config["model"]["variables"]["var_smoothing"]))
        ...
        logging.info("Model and preprocessor saved successfully. Model path: {}, Preprocessor path: {}".format(config["artifacts"]["model_path"], config["artifacts"]["preprocessor_path"]))

        mlflow.log_artifact("config/model1.yaml")
        mlflow.log_artifact(config["artifacts"]["preprocessor_path"])

        logging.info("Artifacts logged to MLflow: config/model1.yaml and {}".format(config["artifacts"]["preprocessor_path"]))
